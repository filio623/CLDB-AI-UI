...
function App() {
  ...
  const getKPIData = () => {
    ...
    kpiMetrics.forEach(kpi => {
      if (metrics[kpi]) {
        // Assuming metrics_comparison structure like: { "ad_displays": { "12345": 2766618, "67890": 2438018 } }
        const campaignIds = Object.keys(metrics[kpi]);
        if (campaignIds.length >= 2) {
          const current = metrics[kpi][campaignIds[0]];
          const previous = metrics[kpi][campaignIds[1]];
